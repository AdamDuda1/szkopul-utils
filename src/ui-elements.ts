...
export function appendProblemSetMenu(addToTODOAction: (id: string) => Promise<boolean> = async () => false) {
	...
	for (let i = rows.length - 1; i >= 0; i--) {
		...
		const id = match?.[1];
		const name = secondTd?.querySelector('a')?.innerText;
		...
		if (id != undefined) {
			...
			cell.querySelector<HTMLAnchorElement>('.js-todo')?.addEventListener('click', (event) => {
				...
				addToTODOAction(`todo: ${id}`).then((result) => {
					if (target) {
						if (result) {
							target.innerHTML = `
								<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-card-checklist" viewBox="0 0 16 16">
